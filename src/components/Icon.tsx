...
/**
 * Lightweight inline icon set (lucide-style strokes) so the site ships zero
 * icon-library JS. Service icons are picked by name in the admin panel, so
 * every name here is a valid choice for the services manager.
 */

type IconDef = { paths: React.ReactNode; filled?: boolean };
...
const ICONS: Record<string, IconDef> = {
  ...
  palette: {
    paths: (
      <>
        <path d="M9.06 11.9 17.13 3.83a2.85 2.85 0 1 1 4.03 4.03l-8.06 8.08" />
...
export default function Icon({ name, size = 24, ...props }: IconProps) {
  const def = ICONS[name] ?? ICONS.globe;
  return (
    <svg
